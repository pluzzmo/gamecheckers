...
public class Board {
	...
	public int moveTo(Player player, Player playerOpposing, Point from,
			Point to, int playerInTurn, boolean isSpecialTurn, Piece specialTurn) {
		ArrayList<Point> moves = null;
		Piece piece = null;
		if (isSpecialTurn != true) {
			piece = player.getPiece(from.x, from.y);
		} else {
			piece = specialTurn;
		}
		int x = 0;
		...
		for (int i = 0; i < moves.size(); i++) {
			if (moves.get(i).x == to.x && moves.get(i).y == to.y) {
				...
				if (player.playerType == Player.PLAYER_BLACK
						&& to.x == board.length - 1) {
					isDama = true;
				} else if (player.playerType == Player.PLAYER_WHITE
						&& to.x == 0) {
					isDama = true;
				}
